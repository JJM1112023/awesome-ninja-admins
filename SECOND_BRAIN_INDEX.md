...
| `always-on-agents` | Design and operate bounded, observable, least-privilege background agents for AS-OS. Use for scheduled Claude Code work, MCP-connected automation, recurring monitoring, handoffs, retries, budgets, and approval gates. |
| `as-os-build-guard` | Automatically protect this AS-OS second-brain build from material changes in MCP, Claude/OpenAI APIs and models, Copilot, GitHub Actions, Replit, Android tooling, authentication, pricing, limits, and automation. Use whenever planning, changing, testing, deploying, reviewing, or resuming those surfaces. |
...
| `claude-security` | Secure Claude Code projects with current permission modes, scoped rules, protected data boundaries, MCP review, secret handling, prompt-injection defenses, and approval-gated external actions. |
